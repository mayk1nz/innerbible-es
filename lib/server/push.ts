...
async function setup(): Promise<void> {
  if (ready) return
  const { data } = await db().from('app_config').select('value').eq('key', 'vapid_private_key').single()
...
export async function removeSubscription(email: string, endpoint: string): Promise<void> {
  await db().from('push_subscriptions').delete().eq('endpoint', endpoint).eq('email', email)
}
...
export async function sendDailyReminders(pick: (email: string) => Reminder): Promise<{ sent: number; removed: number; failed: number }> {
  ...
  for (const s of data ?? []) {
    try {
      ...
      await db().from('push_subscriptions').update({ last_sent_day: today }).eq('endpoint', s.endpoint)
      sent++
    } catch (e) {
