...
export async function consejeroStatus(email: string): Promise<ConsejeroStatus> {
  const [owned, usage] = await Promise.all([
    ownedOffers(email),
    db().from('consejero_usage').select('count').eq('email', email).eq('day', today()).maybeSingle(),
  ])
  const member = owned.includes('upsell2')
  let offerStartedAt: string | null = null
  if (!member) {
    const { data } = await db().from('members').select('offer_started_at').eq('email', email).maybeSingle()
    ...
    if (!offerStartedAt) {
      ...
      await db().from('members').upsert({ email, offer_started_at: offerStartedAt }, { onConflict: 'email' })
    }
  }
...
export async function countMessage(email: string): Promise<void> {
  await db().rpc('consejero_count', { p_email: email, p_day: today() })
}
...
export async function saveMessage(email: string, role: 'user' | 'assistant', content: string, crisis = false): Promise<void> {
  await db().from('consejero_messages').insert({ email, role, content, crisis })
}

export async function clearMessages(email: string): Promise<void> {
  await db().from('consejero_messages').delete().eq('email', email)
}
