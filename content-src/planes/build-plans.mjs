...
for (const plan of PLANS) {
  ...
  const problems = []
  days.forEach((d, i) => {
    if (d.day !== i + 1) problems.push(`día ${i + 1} falta o está repetido (encontré ${d.day})`)
    if (!d.title || !d.versiculo?.texto || !d.versiculo?.referencia || !d.resumen?.length || !d.tarea || d.practica?.length !== 3 || !d.meditar)
      problems.push(`día ${d.day}: campos incompletos`)
  })
  if (days.length !== 90) problems.push(`${days.length} días (esperaba 90)`)
  if (!days.length) throw new Error(`${plan}: no encontré los días en ${dir} — no se escribe nada`)
...
const NAMES = { transformacion: 'Transformación Espiritual', 'vivir-como-jesus': 'Vivir como Jesús', 'nueva-mentalidad': 'Nueva mentalidad' }
const mapLines = []
for (const plan of PLANS) {
  ...
  mapLines.push(`## ${NAMES[plan]} (plan de 90 días de Palabras del Señor)`)
  titles[plan].forEach((t, i) => {
    const idea = outline.find((o) => o.day === i + 1)?.idea ?? ''
    mapLines.push(`- Día ${i + 1}: ${t}${idea ? ` — ${idea.replace(/\s*\(Incluir línea pastoral\)\s*/g, ' ').trim()}` : ''}`)
  })
}
...
console.log(`map.ts escrito (${mapLines.length} linhas, ~${Math.round(mapLines.join('\n').length / 4)} tokens)`)
