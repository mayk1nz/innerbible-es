...
if (offer) {
  ...
  if (arg === 'revoke') {
    await client.query('delete from public.entitlements where email = $1 and offer = $2', [email, offer])
    console.log(`removido: ${email} ${offer}`)
  } else {
    const days = Number(arg ?? 31)
    await client.query(
      `insert into public.entitlements (email, offer, status, current_period_end, source, updated_at)
...
const rows = (await client.query('select offer, status, current_period_end, source from public.entitlements where email = $1 order by offer', [email])).rows
