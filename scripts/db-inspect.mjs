...
const ev = await client.query('select id, received_at, event, email, product, signature_ok, note from public.kashpay_events order by id desc limit 15')
console.log('EVENTOS')
...
const en = await client.query('select email, offer, status, current_period_end, source from public.entitlements order by updated_at desc limit 15')
console.log('ACESSOS')
...
if (process.argv.includes('--payload')) {
  const r = await client.query('select id, headers, payload from public.kashpay_events order by id desc limit 3')
...
if (process.argv.includes('--clean-test')) {
  for (const t of ['kashpay_events', 'entitlements', 'members', 'consejero_messages', 'consejero_usage']) {
    const r = await client.query(`delete from public.${t} where email like '%@example.com'`)
