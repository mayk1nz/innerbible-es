  ...
  if (!src) {
    return (
      <div className="flex items-center gap-4 rounded-3xl border border-line bg-surface p-4">
        ...
        <div>
          <p className="font-semibold text-ink">Audio en preparación</p>
          <p className="text-[14.5px] leading-snug text-muted">Muy pronto podrás escuchar esta lección aquí.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="rounded-3xl bg-primary p-5 text-white shadow-float">
      ...
      <div className="mt-3 flex items-center justify-between">
        <button type="button" onClick={cycleRate} aria-label={`Velocidad ${rate}x`} className="min-w-14 rounded-full border border-white/25 px-3 py-2 text-sm font-semibold hover:bg-white/10">
          {rate}x
        </button>
        <div className="flex items-center gap-3">
          <button type="button" onClick={() => seekBy(-15)} aria-label="Retroceder 15 segundos" className="grid size-11 place-items-center rounded-full hover:bg-white/10">
            <Icon name="rewind" className="size-6" />
          </button>
          <button type="button" onClick={toggle} aria-label={playing ? 'Pausar' : 'Reproducir'} className="grid size-16 place-items-center rounded-full bg-gold-bright text-primary transition active:scale-95">
            <Icon name={playing ? 'pause' : 'play'} className="size-7" />
          </button>
          <button type="button" onClick={() => seekBy(15)} aria-label="Adelantar 15 segundos" className="grid size-11 place-items-center rounded-full hover:bg-white/10">
