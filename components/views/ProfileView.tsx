  ...
  if (!editing) {
    return (
      <button type="button" onClick={() => { setValue(name); setEditing(true) }} className="group flex items-center gap-2 text-left">
        ...
        <Icon name="pencil" className="size-4 text-muted group-hover:text-ink" label="Cambiar nombre" />
      </button>
    )
  }
  return (
  ...
  return (
    <>
      ...
      <a href={`mailto:${APP.supportEmail}`} className="block transition hover:bg-surface-hover">
        <Row icon="mail" title="Soporte" hint={APP.supportEmail}>
          <Icon name="chevronRight" className="size-5 text-muted" />
        </Row>
      </a>
    </>
...
function ManageSubscription({ annual }: { annual: boolean }) {
  ...
  const mail = `mailto:${APP.supportEmail}?subject=${encodeURIComponent('Gestionar mi suscripción')}&body=${encodeURIComponent(`Hola, mi correo de compra es ${session?.email ?? ''}. Quiero `)}`
  return (
    <div className="rounded-2xl border border-line bg-surface">
      <button type="button" aria-expanded={open} onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-3 px-4 py-3.5 text-left">
        <span className="flex-1 text-[15.5px] font-medium text-ink">Gestionar mi suscripción</span>
        ...
        <div className="animate-rise border-t border-line-soft px-4 pb-4 pt-3 text-[15px] leading-relaxed text-text">
          {!annual && (
            <div className="mb-3 rounded-2xl bg-gold-soft/60 p-3.5">
              <p className="font-semibold text-ink">¿Pensando en pausar? Antes, mira esto:</p>
              <p className="mt-1">
                Con el plan anual tienes todo por {formatUsd(ANUAL.price)} al año, mucho menos que mes a mes.
              </p>
              <Link href="/anual" className="mt-2 inline-flex items-center gap-1 font-semibold text-primary underline-offset-4 hover:underline">
                Ver el plan anual
                <Icon name="arrowRight" className="size-4" />
              </Link>
            </div>
          )}
          <p>Para cambiar de plan, actualizar tu tarjeta o cancelar, escríbenos y lo hacemos por ti.</p>
    ...
    <>
      ...
      {s.annual ? (
        <p className="mb-3 flex items-center gap-2 rounded-2xl bg-success-soft px-4 py-3 text-[15px] font-semibold text-success">
          ...
          Plan anual activo: tienes todo incluido.
        </p>
      ) : (
        <Link href="/anual" className="mb-3 flex items-center gap-3.5 rounded-2xl bg-primary p-4 text-white shadow-card">
          <span className="grid size-10 shrink-0 place-items-center rounded-xl bg-white/10 text-gold-bright">
            <Icon name="sparkles" className="size-5" />
          </span>
          <span className="min-w-0 flex-1">
            <span className="block font-semibold">{everything ? 'Pasa al plan anual' : 'Todo incluido por un año'}</span>
            <span className="mt-0.5 block text-[13.5px] leading-snug text-white/80">
              {everything ? `Lo mismo que ya tienes, por ${formatUsd(ANUAL.price)} al año en lugar de pagar cada mes.` : `Todos los recorridos, el Consejero y los planes por ${formatUsd(ANUAL.price)} al año.`}
            </span>
          </span>
      ...
      <div className="divide-y divide-line-soft rounded-3xl border border-line bg-surface">
        <Row icon="moon" title="Modo oscuro" hint="Más cómodo para leer de noche.">
          <Switch on={s.theme === 'dark'} onChange={(on) => setTheme(on ? 'dark' : 'light')} label="Modo oscuro" />
        </Row>
        <NotificationsRow />
        <div className="px-4 py-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-[16px] font-medium text-ink">Tamaño del texto</p>
          ...
          <p className="mt-1 text-[14px] font-semibold text-gold">Salmos 119:105</p>
        </div>
      </div>

      <SectionTitle>App y ayuda</SectionTitle>
