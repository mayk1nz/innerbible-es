...
function contentsLine(product: Product): string {
  if (product.kind === 'enlace') return 'Acceso al grupo'
  return plural(allLessons(product).length, 'lección', 'lecciones')
}
...
export function ProductHeroCard({ product, completed }: { product: Product; completed: AppState['completed'] }) {
  ...
  const cta = !next ? 'Repasar' : started ? 'Continuar' : product.sections[0]?.lessons[0]?.format === 'audio' ? 'Empezar a escuchar' : 'Empezar a leer'

  return (
    <article className="overflow-hidden rounded-3xl border border-line bg-surface shadow-card">
      <Link href={`/modulo/${product.id}`} aria-label={product.title} tabIndex={-1}>
      ...
      <div className="p-5">
        ...
        <p className="mt-1.5 text-[14px] text-muted">{progress.done} de {plural(progress.total, 'lección', 'lecciones')}</p>
        <Link href={next ? lessonHref(product.id, next.lesson.id) : `/modulo/${product.id}`} className={`${buttonClass.primary} mt-4`}>
          {cta}
          <Icon name="arrowRight" className="size-5 text-gold-bright" />
        </Link>
        {next && started && <p className="mt-2.5 text-center text-[14px] text-muted">Sigue: {next.lesson.title}</p>}
      </div>
    </article>
  )
}
  ...
  return (
    <article id={offer.id} className="scroll-mt-6 overflow-hidden rounded-3xl border border-line bg-surface shadow-card">
      ...
      <div className="p-5">
        ...
        <p className="mt-5 text-[13px] font-semibold uppercase tracking-[0.08em] text-gold">Incluye</p>
        ...
        <p className="mt-3 text-center text-[14px] text-muted">Compra con el mismo correo de tu cuenta y se activa solo.</p>
      </div>
    </article>
  )
}
            ...
            <ul className="mt-5 divide-y divide-line-soft rounded-2xl border border-line-soft bg-surface-2">
              {product.sections.map((s) => (
                <li key={s.id} className="flex items-center justify-between gap-3 px-4 py-3 text-[15.5px]">
                  ...
                  <span className="shrink-0 text-muted">{plural(s.lessons.length, 'lección', 'lecciones')}</span>
                </li>
              ))}
            </ul>
          )}
          <p className="mt-5 rounded-2xl bg-gold-soft/60 p-4 text-[15.5px] leading-relaxed text-ink">
            Este contenido es parte de <strong>{offer.title}</strong>.
          </p>
          <div className="mt-4">
            <CheckoutButton offer={offer} email={email} />
          </div>
          <Link href={`/tienda#${offer.id}`} className={`${buttonClass.ghost} mt-2`}>
