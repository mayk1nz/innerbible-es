...
export function MiniPlayer() {
  ...
  if (pathname === href || pathname === '/consejero') return null
    ...
    <>
      ...
      >
        <div className="relative mx-auto flex max-w-[460px] items-center gap-2.5 overflow-hidden rounded-2xl border border-line bg-surface/95 py-2 pl-2 pr-1.5 shadow-[0_10px_30px_-12px_rgb(53_38_15/0.45)] backdrop-blur">
          ...
          <button type="button" onClick={previous} disabled={!hasNeighbour(-1) && p.time <= 5} aria-label="Anterior" className="grid size-9 shrink-0 place-items-center rounded-full text-ink hover:bg-surface-hover disabled:opacity-30">
            <Icon name="skipBack" className="size-5" />
          </button>
          <button type="button" onClick={toggle} aria-label={p.playing ? 'Pausar' : 'Reproducir'} className="grid size-11 shrink-0 place-items-center rounded-full bg-primary text-gold-bright transition active:scale-95">
            <Icon name={p.playing ? 'pause' : 'play'} className="size-5" />
          </button>
          <button type="button" onClick={next} disabled={!hasNeighbour(1)} aria-label="Siguiente" className="grid size-9 shrink-0 place-items-center rounded-full text-ink hover:bg-surface-hover disabled:opacity-30">
            <Icon name="skipForward" className="size-5" />
          </button>
          <button type="button" onClick={closePlayer} aria-label="Cerrar el reproductor" className="grid size-9 shrink-0 place-items-center rounded-full text-muted hover:bg-surface-hover hover:text-ink">
