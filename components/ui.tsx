...
export function SearchInput({
  ...
  return (
    <label className="flex min-h-13 items-center gap-3 rounded-2xl border border-line bg-surface-2 px-4 focus-within:border-primary">
      ...
      />
      {value && (
        <button type="button" onClick={() => onChange('')} aria-label="Borrar búsqueda" className="-mr-2 grid size-9 place-items-center rounded-full text-muted hover:bg-surface">
...
export function FontScaleControl({ scale }: { scale: number }) {
  return (
    <div role="group" aria-label="Tamaño del texto" className="flex items-center rounded-full border border-line bg-surface-2">
      <button
        type="button"
