...
function Countdown({ deadline, now }: { deadline: number; now: number }) {
  ...
  const parts: [string, string][] = [
    ...
    [pad(Math.floor((ms % 86_400_000) / 3_600_000)), 'h'],
    [pad(Math.floor((ms % 3_600_000) / 60_000)), 'min'],
    [pad(Math.floor((ms % 60_000) / 1000)), 's'],
  ]
  return (
    <div className="mt-3 flex items-center justify-center gap-2 font-sans tabular-nums" role="timer" aria-label={`La oferta termina en ${days} días`}>
      {parts.map(([v, unit]) => (
    ...
    <div className="relative overflow-hidden rounded-3xl bg-primary p-5 text-center text-white shadow-float">
      ...
      <div className="relative">
        {active ? (
          ...
          <p className="text-[13px] font-semibold uppercase tracking-[0.08em] text-gold-bright">Desbloquea ahora</p>
        )}
        ...
        <p className="mt-1 text-[15px] text-white/80">
          ...
          <strong className="text-[24px] text-white">{formatUsd(price)}</strong> al mes
        </p>
        {url ? (
          <a
        ...
        {active && (
          <p className="mt-3 text-[13px] leading-snug text-white/75">
            Pagas {formatUsd(deal.discountPrice)} al mes mientras mantengas tu suscripción. Cuando termine el plazo, el precio vuelve a {formatUsd(deal.fullPrice)}.
          </p>
        )}
        <p className="mt-2 text-[13px] text-white/75">
          Compra con el mismo correo de tu cuenta y se activa solo.{deal.note ? ` ${deal.note}` : ''}
        </p>
      </div>
    </div>
  )
...
export function BenefitList({ items, label = 'Todo lo que recibes' }: { items: readonly string[]; label?: string }) {
  return (
    <div>
...
export function DealBadge({ offer }: { offer: UpsellId }) {
  ...
  return <span className="rounded-full bg-danger px-2 py-0.5 text-[11.5px] font-bold uppercase tracking-wide text-white">50% off</span>
}
